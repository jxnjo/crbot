...
def fmt_open_decks_overview(rr: Dict[str, Any], my_tag_nohash: str, max_decks: int = 4) -> str:
    ...
    if (my.get("tag") or "").upper().lstrip("#") != my_tag:
        # Fallback, falls 'clan' fehlt oder nicht unser Tag ist
    ...
    # offene zuerst (nach remaining desc, dann used asc, dann name), dann fertige alphabetisch
    ...
    # Zeitstempel
    try:
        ts = datetime.now(LOCAL_TZ).strftime("%H:%M:%S %Z")  # falls du den LOCAL_TZ-Fallback drin hast
    except Exception:
...
def fmt_activity_list(members_payload: Dict[str, Any]) -> str:
    ...
    Nutzt das Feld 'lastSeen' aus /v1/clans/%23TAG/members.
    """
    items = members_payload.get("items") or []
    rows = []
    for m in items:
        name = m.get("name", "Unbekannt")
        role = m.get("role", "")
        dt = parse_sc_time(m.get("lastSeen"))  # kann None sein
        # None -> sehr alt einsortieren
